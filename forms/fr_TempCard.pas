...
uses
  ...
  Vcl.ComCtrls, Vcl.StdCtrls, TempCard;
...
begin
  lbIGNTimeVal.Caption := IntToStr(TrackIGNTime.Position) + ' min';
end;
...
begin
  ...
  lbSpeedVal.Caption := Format('%.1f km/h', [speedF]);
end;

procedure TfrTempCard.TrackWorkTimeChange(Sender: TObject);
begin
  lbWorkTimeVal.Caption := IntToStr(TrackWorkTime.Position) + ' min';
end;
