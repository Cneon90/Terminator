...
begin
 if fr_main.dbinitialTables then fr_main.sendMessage('Таблицы успешно созданы')
                            else fr_main.sendMessage('Не удалось создать таблицы');
end;
...
begin
  ...
  changeSetting := false;
  fr_main.sendMessage('Настройки сохранены');
end;
...
begin
  ...
  begin
    ...
    btnShowPassword.hint := 'Показать пароль';
  end else
  begin
    edPassword.PasswordChar:= #0;
    btnShowPassword.ImageIndex := 4;
    btnShowPassword.hint := 'Скрыть пароль';
  end;
...
begin
  if changeSetting and (MessageDlg('Настройки не сохранены'+#13+'Хотите продолжить?', mtConfirmation, [mbYes, mbNo], 0) = mrNo)
    then CanClose := false;
end;
